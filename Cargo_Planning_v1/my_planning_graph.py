...
class PlanningGraph():
    ...
    def add_action_level(self, level):
        ...
        # Initialize set of action levels
        a_nodes_set = []

        # Iterate through possible actions and build action nodes
        for action in self.all_actions:
            a_node = PgNode_a(action)

            if a_node.prenodes.issubset(self.s_levels[level]):
                a_nodes_set.append(a_node)
                # Connect S & A nodes by making this action node a child of the preceding level's state nodes
                for s_node in self.s_levels[level]:
                        s_node.children.add(a_node)
                        a_node.parents.add(s_node)

        # Build the action level by appending a_nodes_set to object
        self.a_levels.append(a_nodes_set)
    ...
    def add_literal_level(self, level):
        ...
        # Initialize set of s_nodes
        s_nodes_set = set()

        # Determine all possible literal S nodes in a given A level
        for a_node in self.a_levels[level - 1]:
            # Connect the A & S nodes by making the effect node a child of the preceding action node
            for s_node in a_node.effnodes:
                    s_nodes_set.add(s_node)
                    s_node.parents.add(a_node)
                    a_node.children.add(s_node)

        # Build state level by appending s_nodes_set to object
    ...
    def inconsistent_effects_mutex(self, node_a1: PgNode_a, node_a2: PgNode_a) -> bool:
        ...
        :return: bool
        """
        # Validate that node_a2's effects do not negate any node_a1 effects
        for effect in node_a1.action.effect_add:
            if effect in node_a2.action.effect_rem:
                return True

        # Validate that node_a1's effects do not negate any node_a2 effects
    ...
    def interference_mutex(self, node_a1: PgNode_a, node_a2: PgNode_a) -> bool:
        ...
        # Validate that node_a1's effects do not conflict with node_a2's preconditions
        for effect in node_a1.action.effect_add:
        ...
        # Validate that node_a2's effects do not conflict with node_a1's preconditions
        for effect in node_a2.action.effect_add:
    ...
    def h_levelsum(self) -> int:
        ...
        # For each goal, iterate through all literal // action levels until goal found
        for goal in self.problem.goal:
            goal_node = PgNode_s(symbol=goal, is_pos=True)
            for level, s_nodes in enumerate(self.s_levels):
                # When goal found, add level cost to level sum & break
